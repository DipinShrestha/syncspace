...
export default function DocumentList({ workspaceId, onSelectDocument, selectedDocId }: DocumentListProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await createDocument({ title: newTitle, content: JSON.stringify({ type: 'doc', content: [{ type: 'paragraph' }] }), workspaceId });
  ...
  return (
    <div className="w-64 bg-white border-r p-2 overflow-y-auto h-full">
      <button onClick={() => setIsCreating(true)} className="w-full mb-3 bg-blue-600 text-white py-2 rounded-md text-sm hover:bg-blue-700">+ New Document</button>
      {isCreating && (
        <div className="mb-3 p-2 border rounded bg-gray-50">
          <input type="text" placeholder="Document title" value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full border rounded p-1 text-sm mb-2" autoFocus onKeyDown={e => e.key === 'Enter' && handleCreate()} />
      ...
      <div className="space-y-1">
        {documents.map(doc => (
          <div key={doc._id} onClick={() => onSelectDocument(doc)} className={`p-2 rounded cursor-pointer hover:bg-gray-100 flex justify-between items-center ${selectedDocId === doc._id ? 'bg-blue-50 border-l-4 border-blue-500' : ''}`}>
            <div className="truncate flex-1">
              <div className="font-medium text-sm truncate">{doc.title}</div>
              <div className="text-xs text-gray-400">{new Date(doc.updatedAt).toLocaleDateString()}</div>
            </div>
            <button onClick={e => handleDelete(doc._id, e)} className="text-gray-400 hover:text-red-500">🗑️</button>
          </div>
        ))}
      </div>
    </div>
